...
/**
 * This class provides a mechanism for executing a Java class in a new process.
 * <p>
 * Known limitations:
 * <ul>
...
public class JavaExec {
	...
	/**
	 * @param classpath
	 *            the classpath to be included (may be null)
	 * @param javaProps
	 *            the properties to be passed (may be null)
	 * @param mainClass
	 *            the main class
	 * @param params
	 *            parameters to be passed to the main method (may be null)
	 * @param env
	 *            the application environment (may be null)
	 * @param userDir
	 *            the directory from which the application is to be started (may
	 *            be null)
	 * @return Process
	 * @throws IOException
	 */
	public static Process exec(String[] classpath, Properties javaProps,
			String mainClass, String[] params, String[] env, File userDir)
			throws IOException {
		id++;
		...
		if (javaProps != null) {
			Enumeration<Object> keys = javaProps.keys();
	...
	// Process p =
	// exec(null,null,"ajmas74.experiments.JavaExec",null,null,null);
	// ProcessOutputHandler outHandler = new
	// ProcessOutputHandler(System.out,p.getInputStream(),id);
	// ProcessOutputHandler errHandler = new
	// ProcessOutputHandler(System.err,p.getErrorStream(),id);
	// p.waitFor();
	// System.out.println("----- " + id + " -----");
	// p = exec(null,null,"ajmas74.experiments.JavaExec",null,null,new
	// File("c:/"));
	...
	// p = exec(null,props,"ajmas74.experiments.JavaExec",null,null,new
	// File("c:/"));
	// outHandler = new ProcessOutputHandler(System.out,p.getInputStream(),id);
	// errHandler = new ProcessOutputHandler(System.err,p.getErrorStream(),id);
	// p.waitFor();
