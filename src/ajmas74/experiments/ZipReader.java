...
/**
 * 
 * Note if you want to be able to deal with entries with
 * unicode character names, then you will need to use
 * JDK 1.4.x to write them. Tests with JDK 1.3.x show that
 * it is unable to create unicode file names and folders.
 * This is at least true for MS-Windows, though I have not
 * tested this for other platforms.
 * 
...
//        }
        ...
        try {
          ...
          fout = new FileOutputStream(f);
          containsUTF = true;
          convertedEntriesMap.put(entry.getName(),f.getAbsolutePath());
        }
        ...
        System.out.print(" entry '" + entry.getName() + "' written to: " + f);
        if ( containsUTF ) {
          System.out.println( " - converted file name since it contained characters that " 
  ...
  public static void main ( String[] args ) {
    ...
    if ( args.length >= 2 ) {
      ...
      if ( args[0].equals("-x") ) {
        extractContents(new File((String)files.get(0)),new File("d:/temp/out"));        
